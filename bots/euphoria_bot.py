...
class ZobristHash:
    def __init__(self, rows, cols):
        ...
        for i in range(rows):
            for j in range(cols):
                for p in Player:
                    for c in range(16):   # counts up to 15
...
# Global transposition table (cleared per move)
transposition_table = {}
...
class Board:
    ...
    def evaluate(self, player):
        ...
        score = 0
        EXPLOSION_WEIGHT = 8   # quadratic explosion bonus
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                ...
                if cell.player == Player.NONE or cell.player == player:
                    # Priority: corner (3), edge (2), interior (1) + count/10
                    if (i == 0 or i == self.rows-1) and (j == 0 or j == self.cols-1):
                        priority = 3 + cell.count * 0.1
                    elif i == 0 or i == self.rows-1 or j == 0 or j == self.cols-1:
                        priority = 2 + cell.count * 0.1
                    else:
                        priority = 1 + cell.count * 0.1
                    moves.append((priority, i, j))
        moves.sort(reverse=True)
        return [(i, j) for _, i, j in moves]


# ---------- MINIMAX with TRANSPOSITION TABLE (depth 3) ----------
...
while True:
    line = input()
    board = Board(line)
    best_move = getBestMove(board, depth=3)   # depth 3 is extremely fast
    if best_move:
