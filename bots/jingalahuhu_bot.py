...
class Board:
    ...
    def evaluate(self, player):
        ...
        score = (
            ...
            + 2 * (player_orbs - opponent_orbs)
            + 6 * (player_critical - opponent_critical)
            + 3 * corner_score
            + 2 * edge_score
        )
        ...
        for i in range(self.rows):
            for j in range(self.cols):
                ...
                if cell.player == Player.NONE or cell.player == player:
                    ...
                    # prioritize near explosions
                    if cell.count > 0:
                        moves.append((i, j))
                    else:
                        # only allow some empty cells
                        if (i+j) % 2 == 0:
                            moves.append((i, j))
...
def minimax(board, depth, alpha, beta, maximizing_player, player, move_counter):
    ...
    if maximizing_player:
        ...
        legal_moves.sort(key=lambda move: board.board[move[0]][move[1]].count,reverse=True)
        ...
        for move in legal_moves:
            ...
            if new_board.isTerminal(move_counter+1):
